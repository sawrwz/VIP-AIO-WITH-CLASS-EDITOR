...
public:
	...
	BLOWFISH(bytes* cipherKey, int keylength);

	//TODO: string encryption functions -> base64
	std::string Encrypt_CBC(std::string data);
	bytes* Encrypt_CBC(bytes* data, int length, int* newlength);
	bytes* Encrypt_ECB(bytes* data, int length, int* newlength);
	void Encrypt_Block(bytes* block, int offset = 0);

	std::string Decrypt_CBC(std::string data);
	bytes* Decrypt_CBC(bytes* data, int length, int* newlength);
	bytes* Decrypt_ECB(bytes* data, int length, int* newlength);
	void Decrypt_Block(bytes* block, int offset = 0);

	void SetRandomIV();
	void SetIV(bytes* newIV);
	bytes* GetIV();
	bool IvSet;

protected:
	void SetupKey(bytes* cipherKey, int length);
	void encipher();
	void decipher();
	unsigned int round(unsigned int a, unsigned int b, unsigned int n);
	void setblock(bytes* block, int offset);
	void getblock(bytes* block, int offset);
	...
	bytes* Crypt_ECB(bytes* data, int length, int* newlength, void (BLOWFISH::* CryptBlock)(bytes*, int offset), bool decrypt);
	bytes* Crypt_CBC(bytes* data, int length, int* newlength, void (BLOWFISH::* CryptBlock)(bytes*, int offset), bool decrypt);
	bytes* padData(bytes* data, int length, int* paddedLength, bool decrypt, bool IvSpace);
	int findPaddingEnd(bytes* data, int length);
